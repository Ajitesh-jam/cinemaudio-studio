...
from tango_new.tango2.tools.torch_tools import wav_to_fbank
...
from tango_new.tango2.audioldm.utils import default_audioldm_config, get_metadata
...
class AudioDiffusion(nn.Module):
    ...
    def encode_text(self, prompt):
        ...
        encoder_hidden_states = self._run_text_encoder(
            input_ids, attention_mask, use_grad=not self.freeze_text_encoder
        )
    ...
    def _run_text_encoder(self, input_ids, attention_mask, use_grad=False):
        """Run text encoder; use CPU on MPS to avoid T5 int64 abs() unsupported on MPS."""
        device = self.text_encoder.device
        if device.type == "mps":
            self.text_encoder = self.text_encoder.cpu()
            input_ids, attention_mask = input_ids.cpu(), attention_mask.cpu()
            use_grad = False  # MPS path always no_grad
        ctx = torch.no_grad() if not use_grad else torch.enable_grad()
        with ctx:
            out = self.text_encoder(input_ids=input_ids, attention_mask=attention_mask)[0]
        if device.type == "mps":
            out = out.to(device)
    ...
    def encode_text_classifier_free(self, prompt, num_samples_per_prompt):
        ...
        prompt_embeds = self._run_text_encoder(input_ids, attention_mask)
        ...
        negative_prompt_embeds = self._run_text_encoder(uncond_input_ids, uncond_attention_mask)
...
import copy
from tango_new.tango2.models import AudioDiffusion
import pandas as pd
